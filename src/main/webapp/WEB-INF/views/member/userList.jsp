		...
		<c:forEach var="dto" items="${list}">
			<tr>                                                                                             
				...
				<c:url var="infoUrl" value="memberInfo.do">
					...
					<c:param name="cntPerPage" value="${pdto.canPerPage}"/>
				</c:url>
...
<script>
		...
		$(".page-item").on("click", ".page-link", function (e) {
		    e.preventDefault(); // 기본 동작 방지 a태그의 링그 이벤트 막아줌
		...
		/* $("#cntPerPage").change(() => {
	      ...
	      // window.location.search()=> 쿼리스트링만 가져옴 ?viewPage=2&searchType=&keyword=&cntPerPage=10
	      $("#cntPerPage").change(() => {
	          var cntVal = $("#cntPerPage").val();
	          alert(cntVal);
